...
{
    /// <summary>
    /// Service class for managing eligibility API operations.
    /// </summary>
    /// <remarks>
    /// Initializes a new instance of the <see cref="EligibilityApiService"/> class.
    /// </remarks>
    /// <param name="uow">The unit of work instance.</param>
    public class EligibilityApiService(IUnitOfWork uow) : IEligibilityApiService
    {
        // Declares a private readonly field for unit of work
        private readonly IUnitOfWork _uow = uow;

        /// <summary>
        /// Validates a product's eligibility based on the provided key-values and product details.
        /// </summary>
        /// <param name="userId">The user ID initiating the validation.</param>
        /// <param name="productId">The product identifier to validate.</param>
        /// <param name="keyValues">A dictionary containing key-value pairs representing the validation factors.</param>
        /// <returns>A <see cref="ValidationResult"/> containing validation status and details.</returns>
        public async Task<ValidationResult> ValidAsyncAPI(int userId, int productId, Dictionary<int, object> keyValues)
        {
            // Filters keyValues to find keys with null or empty values
            var nullOrEmptyKeys = keyValues
            ...
            var pCard = _uow.PcardRepository.Query()
                     .FirstOrDefault(p => p.ProductId == productId) ?? throw new Exception("Pcard not found for the given product.");
        ...
        private static ValidationDetail? Validate(Condition condition, Factor factor, object value, string? datatype = null)
        {
            ...
            switch (condition.ConditionValue)
            {
                ...
                case "In List":
                    ...
                    if (!string.IsNullOrEmpty(factor.Value1))
                    {
                        listValues = factor.Value1.Split(',');
                    }
                ...
                case "Not In List":
                    ...
                    string[] notInListValues = [];

                    if (!string.IsNullOrEmpty(factor.Value1))
                    {
                        notInListValues = factor.Value1.Split(',');
                    }
        ...
        /// <summary>
        /// Executes an API call to retrieve details from a specific node based on the given nodeId.
        /// </summary>
        /// <param name="nodeid">The unique identifier for the node that will be used to fetch the details.</param>
        /// <returns>Returns a Task, representing the asynchronous operation.</returns>
        //private async Task ExcuteApi(int nodeid)
        //{
        //    var GetNodeDetails = _uow.NodeModelRepository.Query().FirstOrDefault(na => na.NodeId == nodeid);

        //    if (GetNodeDetails == null)
        //        throw new Exception("Node details not found");


        //    //var details = _apiService.GetApiDetailsAsync(nodeid);
        //    string url = GetNodeDetails.NodeUrl;
        ...
        //    string loginUrl = jsonObject["login"]?.ToString();
        //}
