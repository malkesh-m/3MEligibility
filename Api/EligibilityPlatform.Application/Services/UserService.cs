...
using EligibilityPlatform.Domain.Helper;
...
namespace EligibilityPlatform.Application.Services
{
    ...
    public class UserService(IUnitOfWork uow, IMapper mapper, IConfiguration configuration, IEmailService emailService, ITokenService tokenService, ILdapService ldapService) : IUserService
    {
        ...
        public List<UserGetModel> GetAll(int entityId)
        {
            // Constructs a LINQ query to join users with their status and entity information.
            var users = from Ur in _uow.UserRepository.Query()
                            // Performs a left join with the UserStatus table.
                        join s in _uow.UserStatusRepository.Query() on Ur.StatusId equals s.StatusId into statusGroup
                        from s in statusGroup.DefaultIfEmpty()
                            // Performs a left join with the Entity table.
                        join E in _uow.EntityRepository.Query() on Ur.EntityId equals E.EntityId into entityGroup
                        from E in entityGroup.DefaultIfEmpty()
                            // Filters out deactivated users (status 3) and users from the specified entity.
                        where Ur.StatusId != 3
                        // Projects the result into a UserGetModel.
                        select new UserGetModel
                        {
                            ...
                            // Maps user picture.
                            UserPicture = Ur.UserPicture,
                            Issuspended = Ur.Issuspended,
                            // Maps status ID.
                            StatusId = Ur.StatusId,
                            // Maps entity name (null if join failed).
                            EntityName = E != null ? E.EntityName : null,
                            // Maps status name (null if join failed).
                            StatusName = s != null ? s.StatusName : null,
                            // Subquery to get groups the user belongs to.
                            Groups = (from ug in _uow.UserGroupRepository.Query()
                                          // Joins user groups with security groups.
                                      join sg in _uow.SecurityGroupRepository.Query() on ug.GroupId equals sg.GroupId
                                      // Filters for groups of the current user.
                                      where ug.UserId == Ur.UserId
